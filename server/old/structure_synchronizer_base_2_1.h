...
protected:
    QByteArray get_structure_hash(uint8_t struct_type, Helpz::DB::Base& db, const QString& db_name = QString());
    QByteArray get_structure_hash_for_all(Helpz::DB::Base& db, const QString& db_name = QString());
    ...
    void add_structure_data(uint8_t struct_type, QDataStream& ds, Helpz::DB::Base& db, const QString& db_name = QString());
    void add_structure_items_data(uint8_t struct_type, const QVector<uint32_t>& id_vect, QDataStream& ds, Helpz::DB::Base& db, const QString& db_name = QString());

    QVector<QPair<uint32_t, uint16_t>> get_structure_hash_vect_by_type(uint8_t struct_type, Helpz::DB::Base& db, const QString& db_name = QString());
private:
    template<typename T>
    QString get_db_list_suffix(uint8_t struct_type, const QVector<uint32_t>& id_vect);

    template<typename T>
    QVector<T> get_db_list(uint8_t struct_type, Helpz::DB::Base& db, const QString& db_name, const QVector<uint32_t>& id_vect);

    void add_structure_template(uint8_t struct_type, QDataStream& ds, Helpz::DB::Base& db, const QString& db_name = QString(), const QVector<uint32_t>& id_vect = {});
    template<typename T>
    QVector<QPair<uint32_t, uint16_t>> get_structure_hash_vect(uint8_t struct_type, Helpz::DB::Base& db, const QString& db_name = QString());
