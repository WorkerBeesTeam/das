...
Protocol::Protocol(Work_Object* work_object) :
    ...
    is_copy_(false),
    log_sync_(new Log_Synchronizer(this)),
...
Ver_2_1::Server::Structure_Synchronizer* Protocol::structure_sync()
{
...
void Protocol::process_message(uint8_t msg_id, uint8_t cmd, QIODevice &data_dev)
{
    ...
    {
        QVector<Old::Log_Value_Item> pack;
        if (!data_dev.isOpen() && !data_dev.open(QIODevice::ReadOnly))
        {
            qWarning() << "process_message Protocol_2_0 LOG_PACK_VALUES fail open device";
            return;
        }
        ...
        uint32_t id, user_id;
        qint64 time_msecs; // Milliseconds

        uint32_t item_id;
        QVariant raw_value, value;

        while (pack_size-- > 0 && ds.status() == QDataStream::Ok)
        {
            ds >> id >> time_msecs >> user_id;
            ds >> item_id >> raw_value >> value;

            if (ds.status() != QDataStream::Ok)
            {
                qWarning() << "process_message Protocol_2_0 LOG_PACK_VALUES fail read item";
                break;
            }
            Old::Log_Value_Item pack_item{id, time_msecs, user_id, item_id, raw_value, value};
            pack.push_back(std::move(pack_item));
        }
        if (!pack.isEmpty())
        {
            log_sync_->process_values_pack(pack);
        }
//        Helpz::apply_parse(data_dev, DATASTREAM_VERSION, &Log_Synchronizer::process_values_pack, &log_sync_);
        break;
    }
    case Cmd::LOG_PACK_EVENTS:
    {
        QVector<Old::Log_Event_Item> pack;
        if (!data_dev.isOpen() && !data_dev.open(QIODevice::ReadOnly))
        {
            qWarning() << "process_message Protocol_2_0 LOG_PACK_EVENTS fail open device";
            return;
        }
        QDataStream ds(&data_dev);
        uint32_t pack_size;
        ...
        {
            ds >> id >> time_msecs >> user_id;
            ds >> type_id >> category >> text;

            if (ds.status() != QDataStream::Ok)
            {
                qWarning() << "process_message Protocol_2_0 LOG_PACK_EVENTS fail read item";
                break;
            }
            Old::Log_Event_Item pack_item{id, time_msecs, user_id, type_id, category, text};
            pack.push_back(std::move(pack_item));
        }
        if (!pack.isEmpty())
        {
            log_sync_->process_events_pack(pack);
        }
//        Helpz::apply_parse(data_dev, DATASTREAM_VERSION, &Log_Synchronizer::process_events_pack, &log_sync_);
        break;
    }

    case Cmd::SET_MODE:                 apply_parse(data_dev, &Protocol::mode_changed); break;
    case Cmd::ADD_STATUS:               apply_parse(data_dev, &Protocol::status_added); break;
    case Cmd::REMOVE_STATUS:            apply_parse(data_dev, &Protocol::status_removed); break;
    case Cmd::SET_DIG_PARAM_VALUES:   apply_parse(data_dev, &Protocol::dig_param_values_changed); break;

    case Cmd::MODIFY_SCHEME:
        Helpz::apply_parse(data_dev, DATASTREAM_VERSION, &Ver_2_1::Server::Structure_Synchronizer::process_modify, &structure_sync_, &data_dev);
...
{
    qDebug().noquote() << title() << "unprocess answer" << int(msg_id) << ' ' << int(cmd);
}
...
void Protocol::auth(const Authentication_Info &info, bool modified, uint8_t msg_id)
{
    ...
    if (authenticated)
    {
        ...
        {
            db()->deferred_clear_status(name()); // TODO: Очищать только если отсутствовал более 15 минут.
        }
    }
    else
    {
