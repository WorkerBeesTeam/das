...
void Elements::generate_answer()
{
    ...
    _msg->text = '*';
    _msg->text += scheme_.title_.toStdString();
    _msg->text += '*';
    ...
    if (has_back_btn_)
        _keyboard->payload.buttons.push_back(makeInlineButtonRow(back_data_, "Назад (<<)"));
...
void Elements::fill_sections()
{
    ...
    for (const Section& sct: sct_vect)
    {
        ...
        _keyboard->payload.buttons.push_back(makeInlineButtonRow(data, sct.name().toStdString()));
    }
}
...
void Elements::fill_section(uint32_t sct_id)
{
    ...
    _msg->text += " - ";
    if (q.next())
        _msg->text += q.value(0).toString().toStdString();
...
void Elements::fill_groups(uint32_t sct_id)
{
    ...
    while (q.next())
    {
        ...
        _keyboard->payload.buttons.push_back(makeInlineButtonRow(data, btn_text));
    }
}
...
void Elements::fill_group_info(uint32_t dig_id)
{
    ...
    if (!scheme_status.status_set_.empty())
    {
        ...
        for (const DIG_Status& status: scheme_status.status_set_)
        {
            ...
            _msg->text += '\n';
            _msg->text += default_status_category_emoji(status_it->second.second);

            status_text = status_it->second.first;
            for (const QString& arg: status.args())
                status_text = status_text.arg(arg);

            _msg->text += status_text.toStdString();
        }
    }

    _msg->text += '\n';
    _msg->text += dev_item_text.toStdString();

    _keyboard->payload.buttons.push_back(makeInlineButtonRow(msg_data_ + ".mode", mode_btn_text));
    _keyboard->payload.buttons.push_back(makeInlineButtonRow(msg_data_ + ".param", "Уставки"));
}
...
void Elements::fill_group_mode(uint32_t dig_id)
{
    ...
    vector<MaxBot::Button::Ptr> mode_btn_vect;
    ...
    _keyboard->payload.buttons.push_back(std::move(mode_btn_vect));
}
...
void Elements::fill_group_param(uint32_t dig_id)
{
    ...
    if (cmd_.size() == 6)
    {
        ...
        while (q.next())
        {
            ...
            _keyboard->payload.buttons.push_back(makeInlineButtonRow(data, text));
        }
    }
    else if (cmd_.size() > 6)
    {
        ...
        {
            _msg->text = "Вы собираетесь изменить уставку \"" + param_title + "\" со значением \"" + param_value + "\"\n\nВведите новое значение уставки:";
            skip_edit_ = true;
        }
        else // Confirm user data
        {
            ...
            _keyboard->payload.buttons.push_back(makeInlineButtonRow(data, "Подтвердить"));
        }
    }
}
