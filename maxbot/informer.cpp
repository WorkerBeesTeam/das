...
void Informer::send_event_messages(const Scheme_Info &scheme, const QVector<Log_Event_Item> &event_pack)
{
    ...
    QVector<MaxBot_Subscriber> subscribers = db_build_list<MaxBot_Subscriber>(db, scheme.scheme_groups(), {},
                                                                      MaxBot_Subscriber::COL_group_id);
    if (subscribers.empty())
        return;

    const std::string data = '*' + scheme_title + "*\n" + text.toStdString();

    for (MaxBot_Subscriber subscriber: subscribers)
    {
...
std::set<int64_t> Informer::get_disabled_chats(uint32_t status_type_id, uint32_t dig_id, const Scheme_Info &scheme)
{
    ...
    sql = R"sql(
SELECT mbc.id FROM das_maxbot_chat mbc
LEFT JOIN das_maxbot_user mbu ON mbu.id = mbc.admin_id
LEFT JOIN das_user_groups ug ON ug.user_id = mbu.user_id
...
Informer::Prepared_Data &Informer::get_prepared_data(const Scheme_Info &scheme)
{
    ...
    if (it == prepared_data_map_.end())
    {
        ...
        for (const MaxBot_Subscriber& ts: subscribers)
            if (ts.chat_id())
