...
class DAS_LIBRARY_SHARED_EXPORT MaxBot_User
{
    HELPZ_DB_META(MaxBot_User, "maxbot_user", "mbu", DB_A(id), DB_AN(user_id), DB_A(first_name),
                  DB_A(last_name), DB_A(user_name), DB_A(lang), DB_AN(private_chat_id))
public:
    MaxBot_User(int32_t id = 0, uint32_t user_id = 0, const QString& first_name = {},
...
using MaxBot_User = DB::MaxBot_User;
