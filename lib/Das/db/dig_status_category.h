...
{
    HELPZ_DB_META(DIG_Status_Category, "dig_status_category", "gsc", DB_A(id), DB_A(name), DB_A(title), DB_AM(color), DB_A(scheme_id))
public:
...
using DIG_Status_Category = DB::DIG_Status_Category;
using DIG_Status_Category_Manager = DB::DIG_Status_Category_Manager;
