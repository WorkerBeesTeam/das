...
Code_Item::Code_Item(uint id, const QString &name, uint32_t global_id, const QString &text) :
    Named_Type(id, name), global_id(global_id), text(text) {}
...
{
    return ds << static_cast<const Named_Type&>(item) << item.global_id << item.text;
}

QDataStream &operator>>(QDataStream &ds, Code_Item &item)
{
    return ds >> static_cast<Named_Type&>(item) >> item.global_id >> item.text;
}
