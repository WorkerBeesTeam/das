...
Worker_Structure_Synchronizer::Worker_Structure_Synchronizer(Worker* worker) :
    Ver_2_4::Structure_Synchronizer_Base(worker->db_pending()),
    worker_(worker)
{
}
...
{
    if (struct_type == Ver_2_4::ST_USER)
    {
        QString sql = Ver_2_4::Client::Structure_Synchronizer::get_scheme_group_users_fill_sql();
    ...
    if (proto)
    {
        proto->send(Ver_2_4::Cmd::MODIFY_SCHEME).writeRawData(buffer.constData(), buffer.size());
    }
