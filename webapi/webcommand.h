...
signals:
    void send(std::shared_ptr<Network::Websocket_Client> client, const QByteArray& data) const;
...
public slots:
private slots:
    void write_command(std::shared_ptr<Network::Websocket_Client> client, uint32_t scheme_id, quint8 cmd, const QByteArray& data);
private:
    WebSocket* websock_;
};
